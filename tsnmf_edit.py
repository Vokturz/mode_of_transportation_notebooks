...
def _initialize_tsnmf(X, L, n_components, init=None, eps=1e-6, random_state=None):
    """
...
def create_constraint_matrix(labels, n_components):
    ...
    """
    L = np.ones((len(labels), n_components))  # initialize matrix of ones
...
class TSNMF:
    ...
    def fit_transform(self, X, labels, y=None, W=None, H=None):
        ...
        W, H, n_iter = topic_supervised_factorization(X, W, H, self.n_components,
                                        labels, init =self.init, tol=self.tol, random_state=self.random_state,
...
def topic_supervised_factorization(X, W=None, H=None, n_components=None,
    ...
    if not update_H:
        ...
        W_init, H_init = _initialize_tsnmf(X, L, n_components, init=init, random_state=random_state)
